...
export default function UserFinancial() {
  ...
  if (!user) return (
    <AdminLayout>
      <p className="p-4 lg:p-6 text-red-500 font-semibold text-center">User not found</p>
    </AdminLayout>
  );
  ...
  const investments = financialData.transactions.filter(t => t.type === "investment");
  const gamingExpenses = expenses.filter(e => e.category === "Gaming");
  const recoveryInvestments = investments.filter(i => i.category === "Recovery");
      ...
      <div className="max-w-6xl mx-auto p-4 lg:p-6 space-y-4 lg:space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 lg:gap-6 mb-6 lg:mb-8">
          <div className="p-4 lg:p-6 bg-green-500/20 rounded-xl border border-green-500/30">
            ...
            <div className="text-xs lg:text-sm text-green-200">Initial Balance</div>
          </div>
          ...
          <div className="p-4 lg:p-6 bg-blue-500/20 rounded-xl border border-blue-500/30">
            ...
            <div className="text-xs lg:text-sm text-blue-200">Current Balance</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
          ...
          <div className="p-4 lg:p-6 bg-white/5 rounded-xl border border-white/10">
            <h3 className="text-lg lg:text-xl font-semibold text-red-300 mb-3 lg:mb-4">Gaming Expenses</h3>
            <div className="space-y-2 lg:space-y-3">
              {gamingExpenses.map(expense => (
                <div key={expense.id} className="flex justify-between items-center p-2 lg:p-3 bg-red-500/10 rounded-lg">
                  <div className="flex-1 min-w-0">
                    <div className="text-white font-medium text-sm lg:text-base truncate">{expense.description}</div>
                    <div className="text-gray-400 text-xs lg:text-sm">{expense.date}</div>
                  </div>
              ...
              <div className="flex justify-between items-center pt-2 lg:pt-3 border-t border-white/10">
                <div className="text-white font-semibold text-sm lg:text-base">Total Gaming Expenses</div>
                <div className="text-red-300 font-bold text-sm lg:text-base">
                  -₹{gamingExpenses.reduce((sum, e) => sum + e.amount, 0).toLocaleString()}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 lg:p-6 bg-white/5 rounded-xl border border-white/10">
            <h3 className="text-lg lg:text-xl font-semibold text-green-300 mb-3 lg:mb-4">Recovery Investments</h3>
            <div className="space-y-2 lg:space-y-3">
              {recoveryInvestments.map(investment => (
                <div key={investment.id} className="flex justify-between items-center p-2 lg:p-3 bg-green-500/10 rounded-lg">
                  <div className="flex-1 min-w-0">
                    <div className="text-white font-medium text-sm lg:text-base truncate">{investment.description}</div>
                    <div className="text-gray-400 text-xs lg:text-sm">{investment.date}</div>
                  </div>
              ...
              <div className="flex justify-between items-center pt-2 lg:pt-3 border-t border-white/10">
                <div className="text-white font-semibold text-sm lg:text-base">Total Recovery Investments</div>
                <div className="text-green-300 font-bold text-sm lg:text-base">
                  -₹{recoveryInvestments.reduce((sum, i) => sum + i.amount, 0).toLocaleString()}
                </div>
              </div>
            </div>
          </div>
        ...
        {/* All Transactions */}
        <div className="p-4 lg:p-6 bg-white/5 rounded-xl border border-white/10">
          <h3 className="text-lg lg:text-xl font-semibold text-indigo-300 mb-3 lg:mb-4">All Transactions</h3>
          <div className="space-y-2">
            {financialData.transactions.map(transaction => (
              <div key={transaction.id} className={`flex justify-between items-center p-2 lg:p-3 rounded-lg ${
                transaction.type === 'expense' ? 'bg-red-500/10' : 'bg-green-500/10'
              }`}>
                <div className="flex items-center gap-2 lg:gap-4 flex-1 min-w-0">
                  <span className={`w-2 h-2 lg:w-3 lg:h-3 rounded-full flex-shrink-0 ${
                    transaction.type === 'expense' ? 'bg-red-500' : 'bg-green-500'
                  }`}></span>
                  <div className="flex-1 min-w-0">
                    <div className="text-white font-medium text-sm lg:text-base truncate">{transaction.description}</div>
                    <div className="text-gray-400 text-xs lg:text-sm">
                      {transaction.date} • {transaction.category}
                    </div>
                  </div>
                </div>
                <div className={`font-bold text-sm lg:text-base ml-2 lg:ml-4 flex-shrink-0 ${
                  transaction.type === 'expense' ? 'text-red-300' : 'text-green-300'
                }`}>
        ...
        {/* Savings Progress */}
        <div className="p-4 lg:p-6 bg-white/5 rounded-xl border border-white/10">
          <h3 className="text-lg lg:text-xl font-semibold text-yellow-300 mb-3 lg:mb-4">Savings Progress</h3>
          <div className="space-y-3 lg:space-y-4">
            <div>
              <div className="flex justify-between text-xs lg:text-sm text-white mb-1">
                <span>Money Saved</span>
                <span>₹{financialData.currentBalance.toLocaleString()} / ₹{financialData.initialBalance.toLocaleString()}</span>
              </div>
              <div className="w-full bg-white/10 rounded-full h-2 lg:h-3">
                <div 
                  ...
                  style={{ 
                    width: `${(financialData.currentBalance / financialData.initialBalance) * 100}%` 
                  }}
                ></div>
              </div>
            </div>
            <div className="text-center text-gray-300 text-sm lg:text-base">
              {((financialData.currentBalance / financialData.initialBalance) * 100).toFixed(1)}% of initial balance saved
            </div>
          </div>
        </div>
      </div>
