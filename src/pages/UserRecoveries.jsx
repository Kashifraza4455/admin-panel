      ...
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <Link 
              to={`/users/${user.id}`}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20 text-white transition mb-4"
            >
              ← Back
            </Link>
        ...
        <div className="p-8 bg-yellow-500/20 border border-yellow-500/30 rounded-xl text-center">
          <div className="text-6xl mb-4">⏳</div>
          <h2 className="text-2xl font-bold text-yellow-300 mb-2">Recovery in Progress</h2>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            ...
            <div className="px-4 py-2 bg-indigo-500/30 rounded-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-4 bg-white/5 rounded-xl border border-white/10 text-center">
            <div className="text-2xl font-bold text-green-300">{user.days}</div>
            <div className="text-sm text-green-200">Days Completed</div>
          </div>
          <div className="p-4 bg-white/5 rounded-xl border border-white/10 text-center">
            <div className="text-2xl font-bold text-blue-300">{user.targetDays - user.days}</div>
...
<div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-white/5 rounded-xl border border-white/10">
  ...
  <div>
    <label className="block text-sm font-semibold text-white mb-2">Filter by Day</label>
    <CustomDropdown
      value={selectedDay}
  ...
  <div className="flex items-end">
    <button
      ...
      className="w-full px-4 py-2 rounded-lg bg-[#4C1B76] hover:bg-[#5A2188] text-white transition border-none font-medium focus:outline-none focus:ring-0"
    >
      Reset Filters
    </button>
  </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="p-4 bg-green-500/20 rounded-xl border border-green-500/30">
          <div className="text-2xl font-bold text-green-300">{recoveryData.length}</div>
          <div className="text-sm text-green-200">Total Days Tracked</div>
        </div>
        <div className="p-4 bg-blue-500/20 rounded-xl border border-blue-500/30">
          <div className="text-2xl font-bold text-blue-300">
            {recoveryData.filter(day => getProgressPercentage(day) === 100).length}
          </div>
          <div className="text-sm text-blue-200">Perfect Days</div>
        </div>
        ...
        <div className="p-4 bg-yellow-500/20 rounded-xl border border-yellow-500/30">
          ...
          <div className="text-sm text-yellow-200">Average Progress</div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {filteredData.length === 0 ? (
          <div className="text-center p-8 bg-white/5 rounded-xl border border-white/10">
            <p className="text-gray-400 text-lg">No recovery data found for the selected filters.</p>
          </div>
        ) : (
          filteredData.map((entry, index) => (
            <div key={index} className="p-6 bg-white/5 rounded-xl border border-white/10 hover:bg-white/10 transition">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                <div className="flex items-center gap-4">
                  <div className="px-4 py-2 bg-indigo-500/20 text-indigo-300 rounded-lg font-bold">
                    Day {entry.day}
                  </div>
                  <div className={`px-3 py-1 rounded-full text-sm border ${getCategoryColor(entry.category)}`}>
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {entry.completedTasks.map((task, taskIndex) => (
                      <li key={taskIndex} className="flex items-center gap-3 p-3 bg-green-500/10 rounded-lg border border-green-500/20">
                  ...
                  {entry.pendingTasks.length > 0 ? (
                    <ul className="space-y-2">
                      {entry.pendingTasks.map((task, taskIndex) => (
                        <li key={taskIndex} className="flex items-center gap-3 p-3 bg-red-500/10 rounded-lg border border-red-500/20">
              ...
              {entry.notes && (
                <div className="mt-4 p-4 bg-blue-500/10 rounded-lg border border-blue-500/20">
                  <h4 className="text-sm font-semibold text-blue-300 mb-2">Daily Notes:</h4>
